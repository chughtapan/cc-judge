...
import { RunnerResolutionCause, RunnerResolutionError } from "../core/errors.js";
...
function buildObservability(
  ...
  if (emitBraintrust) {
    // CLI boundary: environment is converted into typed emitter config here.
  ...
  if (args.bin === undefined) {
    return Effect.fail(
      new RunnerResolutionError({
        cause: RunnerResolutionCause.InvalidRuntime({ value: "subprocess: missing --bin" }),
      }),
    );
  }
...
function runHarnessPlanCommand(args: RunCliArgs): Effect.Effect<CliExitCode, never, never> {
  return resolveRuntimeOverride(args).pipe(
    Effect.flatMap((runtime) => Effect.gen(function* () {
      ...
      const report = yield* runPlannedHarnessPath(args.scenarioPath, {
        judge,
    ...
    Effect.catchTag("RunnerResolutionError", (error) =>
      Effect.sync(() => {
